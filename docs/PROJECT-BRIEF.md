...
| **Journal** | entries with a **Tiptap rich-text editor** (bold/italic/headings/lists/quote); tags; link verses; **quick-capture from Bible & prayer**; **full-text search + tag filtering** | backlinks |
| **Study rail** | Tabbed right rail: **Commentary** (public-domain via HelloAO, tracks chapter, cached), **Cross-references** (OpenBible TSK, 342k, CC-BY — click a verse), **Strong's word-study** (NT Greek from BSB word tags; **OT Hebrew from the Westminster Leningrad Codex / OSHB — accurate interlinear, RTL**) | a personal commentary source; split-pane; PDF; morphology display |
...
| **Plans** | 5 built-in plans + **custom plan builder** (pick a book range + days → generated plan, deletable); progress by chapters read (never "behind"); mark days done; open readings; set active | calendar view |
